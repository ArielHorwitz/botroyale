...
documentation. Will delete the `docs/botroyale` folder and recreate docs.
"""
...
if not ROOT_PACKAGE_NAME.isidentifier():
    raise NameError(
        f'Project directory "{ROOT_PACKAGE_NAME}" must be a valid python '
        "package name. Try renaming your project folder with *only* letters, "
        '(e.g. "botroyale").'
    )
...
class Guide(NamedTuple):
    """Nametuple for a guide for the docs."""
...
def run():
    """Build the documentation using `make_docs`."""
    make_docs()
...
def _rebuild_guides():
    ...
    Reads each guide markdown (.md) file in and writes a python module file
    with a docstring that includes the markdown file (to be indexed by
    `make_docs`). Then writes an `__init__.py` with a docstring with formatted
    links to the html files that will be generated by pdocs.
    """
...
def _find_guide_title_categories(guide_md_file):
    ...
    To find the title, we return the contents of the first *Header 1* in the
    file (a line starting with "# "). Default to the file name if no title found.

    To find categories, we look for a line with a specific format and parse the
    category names. This line must be prepended by `CATEGORY_IDENT_PREFIX` and
    appended by `CATEGORY_IDENT_SUFFIX`. The text in between these affixes will
    be split using the `CATEGORY_IDENT_SPLIT` to find a list of categories.

    For example, if the prefix is "*Categories", the suffix is "*" and the
    split is ", ", then the line "*Categories: beginner, guides*" in a guide
    markdown file will indicate that the guide belongs to the categories
    "beginner" and "guides".
    """
    ...
    for line in markdown.split("\n"):
        if not title and line.startswith("# "):  # A markdown header
            title = line[2:]
        if not categories:
            if line.startswith(CATEGORY_IDENT_PREFIX) and line.endswith(
                CATEGORY_IDENT_SUFFIX
            ):
                parse_this = line[
                    len(CATEGORY_IDENT_PREFIX) : -len(CATEGORY_IDENT_SUFFIX)
                ]
    ...
    for c in categories:
        if c not in GUIDE_CATEGORIES:
            raise KeyError(
                f'Unknown category "{c}" in guide: {guide_md_file.name} ; '
                f"{GUIDE_CATEGORIES=}"
            )
