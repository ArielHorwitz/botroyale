...
MISSING_GIT_ERROR = EnvironmentError(f'Could not find git repo for {PROJ_DIR} (CWD: {Path.cwd()})')
UNCLEAN_GIT_ERROR = RuntimeError(f'Unclean git status, please commit or stash changes. Run `git status` for details.')
...
def upgrade_version(write_tag: bool = False, **kwargs):
    ...
    cmds = [
        ...
        partial(subprocess.run, ['git', 'commit', '-m', f'{SUBJECT_PREFIX}{new_version}']),
    ]
    if write_tag:
        cmds.append(partial(subprocess.run, [
            'git', 'tag', '-a', f'v{new_version}', '-m', f'{SUBJECT_PREFIX}{new_version}']))
...
def undo_upgrade():
    ...
    confirm = f'Ready to undo commit: {get_head_hash()[:6]} {head_subject} (code version: {VERSION})'
    # Check `in` and not `startswith` since the head_subject may be encased in quotation marks
    if not SUBJECT_PREFIX in head_subject:
        confirm = '\n'.join([
            confirm,
            '='*100,
            f'WARNING! Commit suspected to not be a version upgrade commit: {head_subject}',
            '='*100,
        ])
    r = _run_commands([
...
def is_git() -> bool:
    """If the current working directory is at project's root and contains a local git repo."""
    # Confirm it is the correct directory
    if not Path.cwd().samefile(PROJ_DIR):
        return False
    # Confirm there is a git repo
    is_inside_tree = subprocess.run(['git', 'rev-parse', '--is-inside-work-tree'], capture_output=True)
...
def is_git_clean() -> bool:
    ...
    clean_tree_staged = subprocess.run(['git', 'diff', '--cached', '--exit-code'], capture_output=True)
...
def get_head_subject() -> str:
    ...
    if not is_git():
        raise MISSING_GIT_ERROR
    console_output = subprocess.check_output(['git', 'show', '--pretty=format:"%s"', '--no-patch'])
    return console_output.decode('ascii').strip()
...
def _run_commands(commands, confirm_prompt=False):
    print(f'Queued commands:')
    print('\n'.join(f'  {cmd}' for cmd in commands))
    if confirm_prompt:
        print(confirm_prompt)
        confirm = input('Enter "confirm" to proceed (anything else or leave empty to abort): ') == 'confirm'
        if not confirm:
            print('Aborted.')
            return False
    for cmd in commands:
